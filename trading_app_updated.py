...
# SECURITY: never hardcode broker credentials in source. Read from environment
# variables (or Streamlit secrets) instead. Set these in your shell / deployment
# config, e.g.:
#   export KITE_API_KEY="..."
#   export KITE_API_SECRET="..."
# or in Streamlit Cloud: st.secrets["KITE_API_KEY"] / st.secrets["KITE_API_SECRET"]
...
if not KITE_API_KEY or not KITE_API_SECRET:
    st.sidebar.warning(
        "⚠️ KITE_API_KEY / KITE_API_SECRET not found in environment or secrets. "
        "Live broker features (auth, order placement) will be disabled until "
        "these are configured. Never commit real credentials into this file."
    )
...
class AdvancedRiskManager:
    ...
    def calculate_risk_based_position_size(self, available_capital, entry_price, stop_loss,
        ...
        which ignores stop distance entirely and can put very different real
        risk behind two trades of the same "size".

            risk_per_trade  = equity * risk_pct        (e.g. 0.5% of equity)
        ...
        try:
            ...
            risk_per_share = abs(float(entry_price) - float(stop_loss)) if stop_loss else None
            if not risk_per_share or risk_per_share <= 0:
                # No usable stop distance — fall back to a conservative proxy
                # (0.75% of entry price) rather than sizing unbounded.
                risk_per_share = float(entry_price) * 0.0075
            ...
            try:
                return max(0, int((available_capital * risk_pct * 100) / entry_price))
            except Exception:
                return 0
...
class MLSignalEnhancer:
    ...
    def create_ml_features(self, data):
        ...
        try:
            ...
            features['trend_strength'] = data['HTF_Trend'].iloc[-1] if 'HTF_Trend' in data.columns else 0
    ...
    def predict_signal_confidence(self, symbol_data):
        """
        HEURISTIC confidence score, NOT a trained ML model.
        This is a manually-weighted rule-based score (despite the class name).
        It has not been fit or validated against historical outcomes, so treat
        its output as a rough qualitative adjustment only, not a calibrated
        probability. A real version would train a classifier/regressor on the
        feature set in create_ml_features() against labeled trade outcomes
        (e.g. "did price hit +2R before -1R"), see RealBacktestEngine for the
        first step toward that (walk-forward win-rate calculation).
        """
        if not self.enabled:
        ...
        try:
            ...
            # Volume confirmation
            # FIX: branches must be checked from most- to least-restrictive,
            # otherwise the >2.0 case can never be reached (anything >2.0
            # already satisfies >1.5, which is checked first).
...
def session_vwap(df):
    """
    Session-reset VWAP: cumulative typical-price*volume divided by cumulative
    volume, reset at the start of each trading day (by calendar date of the
    index). Using a plain running cumsum over a multi-day download (the old
    behaviour) lets prior sessions' volume contaminate today's VWAP, which is
    wrong for intraday VWAP-based signals.
    """
    typical_price = (df["High"] + df["Low"] + df["Close"]) / 3.0
    tp_vol = typical_price * df["Volume"]
    session_key = df.index.tz_localize(None).date if getattr(df.index, "tz", None) is not None else df.index.date
...
class EnhancedDataManager:
    ...
    def get_stock_data(self, symbol, interval="15m"):
        ...
        # FIX: session-reset VWAP instead of a running cumsum over the whole
        # multi-day download (which lets prior sessions' volume contaminate
        # today's VWAP).
        ...
        # FIX: HTF (higher-timeframe) trend confirmation must default to
        # NEUTRAL (0) on any failure, not bullish (1). HTF_Trend gates BUY
        # signals downstream via `htf_trend == 1`; defaulting to 1 meant a
        # failed/missing 1h fetch silently forged a bullish confirmation.
        # Neutral (0) fails both the `== 1` and `== -1` checks, so a missing
        # confirmation correctly blocks the signal instead of faking one.
        try:
            ...
            if htf is not None and len(htf) > 50:
                ...
                df["HTF_Trend"] = 0  # unavailable -> neutral, not bullish
        except Exception:
            df["HTF_Trend"] = 0  # unavailable -> neutral, not bullish
    ...
    def create_validated_demo_data(self, symbol):
        ...
        df["HTF_Trend"] = 0  # neutral: this is synthetic data, no real HTF confirmation exists
        # SAFETY: this is randomly generated synthetic price data used only so the UI
        # has something to render when live data is unavailable. It must NEVER be
        # allowed to drive a real order. `is_demo=True` is checked and enforced in
        # generate_quality_signals()/auto_execute_signals()/execute_trade() below —
    ...
    def get_historical_accuracy(self, symbol, strategy):
        """
        Real walk-forward historical accuracy (see RealBacktestEngine), not a
        hardcoded lookup table. Falls back to a clearly-labeled prior
        estimate if live data isn't available for this symbol.
        """
        try:
            data = self.get_stock_data(symbol, "15m")
            return self.backtest_engine.calculate_historical_accuracy(symbol, strategy, data)
        except Exception as e:
            logger.error(f"get_historical_accuracy failed for {symbol}/{strategy}: {e}")
            return RealBacktestEngine.PRIOR_ESTIMATES.get(strategy, 0.65)
    ...
    def calculate_optimal_position_size(self, symbol, win_probability, win_loss_ratio, available_capital, price, atr, stop_loss=None, risk_pct=0.005):
        """
        Calculate position size. Primary method is risk-based (risk_pct of
        equity divided by the actual stop distance), capped by a
        fractional-Kelly quantity and a hard concentration ceiling. See
        AdvancedRiskManager.calculate_risk_based_position_size for details.
        Falls back to plain (half-)Kelly sizing only if no stop_loss is
        available at all.
        """
        if stop_loss is not None:
            return self.risk_manager.calculate_risk_based_position_size(
                available_capital, price, stop_loss,
                win_probability=win_probability, win_loss_ratio=win_loss_ratio, risk_pct=risk_pct
            )
...
class RealBacktestEngine:
    """
    Genuine (if simplified) walk-forward backtest, replacing the old fixed
    accuracy_map — see review item #1: a system that prints "82% historical
    accuracy" from a hardcoded number rather than from actual historical
    trades is dangerously misleading.

    Method: replay each strategy's entry condition bar-by-bar over the
    historical window that has ALREADY been fetched (no lookahead — every
    condition uses only data available up to and including that bar), and
    when triggered, simulate forward to see whether a 2R target or 1R stop
    (built the same way calculate_improved_stop_target sizes trades) is hit
    ...
    Known limitations (documented rather than hidden):
      ...
      - The lookback window is whatever get_stock_data fetched (7 days of
        ...
        study — treat the output as "recent local expectancy", not a
        definitive edge, and prefer the blended prior below when the sample
        is thin.
      - This does NOT implement the train/validation/out-of-sample split or
        multi-symbol pooling recommended in the review (items #24) — that
        needs a persistent trade log across sessions/symbols, which is a
        larger project (see RealBacktestEngine.calculate_historical_accuracy
        docstring notes above for what would be needed next).
    """

    MIN_TRADES_FOR_TRUST = 15   # below this, blend towards the static prior
    LOOKAHEAD_BARS = 16         # ~4 hours on 15m bars
    RISK_MULT_STOP = 1.2        # matches calculate_improved_stop_target intent
    RISK_MULT_TARGET = 2.0      # target R multiple simulated

    # Conservative, clearly-labeled PRIOR estimates used only (a) as a
    # Bayesian-style blend when the walk-forward sample is small, or (b) as
    # an emergency fallback if data is unavailable/demo. These are NOT
    # presented as measured accuracy on their own anymore.
    PRIOR_ESTIMATES = {
        "Multi_Confirmation": 0.70, "Enhanced_EMA_VWAP": 0.68, "Volume_Breakout": 0.65,
        "RSI_Divergence": 0.62, "MACD_Trend": 0.60, "EMA_VWAP_Confluence": 0.62,
        "RSI_MeanReversion": 0.58, "Bollinger_Reversion": 0.55, "MACD_Momentum": 0.60,
        "Support_Resistance_Breakout": 0.60, "EMA_VWAP_Downtrend": 0.62,
        "RSI_Overbought": 0.55, "Bollinger_Rejection": 0.53, "MACD_Bearish": 0.58,
        "Trend_Reversal": 0.52,
    }

    def __init__(self):
        self.historical_results = {}   # cache: (symbol, strategy, n_bars, last_ts) -> result dict
    ...
    def _entry_condition(strategy, row, prev_rsi, support=None, resistance=None):
        ...
        try:
            ...
            if strategy in ("EMA_VWAP_Confluence", "Enhanced_EMA_VWAP"):
                if ema8 > ema21 > ema50 and close > vwap and adx_val > 25 and htf_trend == 1:
                    return "BUY"
            if strategy == "RSI_MeanReversion":
                if rsi_val < 30 and close > support and rsi_val > 25:
                    return "BUY"
            if strategy == "Bollinger_Reversion" and bb_lower is not None:
                if close <= bb_lower:
                    return "BUY"
            if strategy in ("MACD_Momentum", "MACD_Trend"):
                if macd_line > macd_signal and ema8 > ema21 and close > vwap:
                    return "BUY"
            if strategy == "EMA_VWAP_Downtrend":
                if ema8 < ema21 < ema50 and close < vwap and adx_val > 25:
                    return "SELL"
            if strategy == "RSI_Overbought":
                if rsi_val > 70 and close < resistance and rsi_val < 75:
                    return "SELL"
            if strategy == "Bollinger_Rejection" and bb_upper is not None:
                if close >= bb_upper:
                    return "SELL"
            if strategy == "MACD_Bearish":
                if macd_line < macd_signal and ema8 < ema21 and close < vwap:
                    return "SELL"
            if strategy in ("Support_Resistance_Breakout", "Volume_Breakout"):
                if vol_avg and vol > vol_avg * 1.5:
                    if close > resistance:
                        return "BUY"
                    if close < support:
                        return "SELL"
            if strategy in ("Trend_Reversal", "RSI_Divergence"):
                if prev_rsi is not None:
                    if rsi_val > 70 and rsi_val < prev_rsi:
                        return "SELL"
                    if rsi_val < 30 and rsi_val > prev_rsi:
                        return "BUY"
        except Exception:
            return None
        return None
    ...
    def _simulate_forward(self, df, entry_idx, action, entry_price, atr):
        ...
        if action == "BUY":
            stop = entry_price - atr * self.RISK_MULT_STOP
            target = entry_price + atr * self.RISK_MULT_STOP * self.RISK_MULT_TARGET
        else:
            stop = entry_price + atr * self.RISK_MULT_STOP
            target = entry_price - atr * self.RISK_MULT_STOP * self.RISK_MULT_TARGET
    ...
    def calculate_historical_accuracy(self, symbol, strategy, data):
        ...
        try:
            ...
            cache_key = (symbol, strategy, len(data))
            cached = self.historical_results.get(cache_key)
            if cached and cached["last_ts"] == last_ts and (time.time() - cached["computed_at"]) < self.cache_ttl_seconds:
            ...
            for i in range(warmup, max(warmup, end)):
                ...
                if outcome == "WIN":
                    ...
                    losses += 1
                # outcome is None (neither hit within lookahead) -> excluded

            total = wins + losses
            prior = self.PRIOR_ESTIMATES.get(strategy, 0.6)

            if total == 0:
                win_rate = prior
            else:
                measured = wins / total
                if total < self.MIN_TRADES_FOR_TRUST:
                    # Blend towards the prior when the sample is thin, weighted
                    # by how much data we actually have (simple shrinkage).
                    weight = total / self.MIN_TRADES_FOR_TRUST
                    win_rate = weight * measured + (1 - weight) * prior
                else:
                    win_rate = measured
            ...
            logger.error(f"Backtest failed for {symbol}/{strategy}: {e}")
            return self.PRIOR_ESTIMATES.get(strategy, 0.6)
...
class MultiStrategyIntradayTrader:
    ...
    def execute_trade(self, symbol, action, quantity, price, stop_loss=None, target=None, win_probability=0.75, auto_trade=False, strategy=None, is_demo_data=False):
        # SAFETY: never let a trade (paper or real) execute against synthetic/
        # demo price data. Demo data is randomly generated (see
        # create_validated_demo_data) purely so the UI has something to show
        # when the live feed is unavailable; it must not silently produce a
        # signal that looks tradeable.
        if is_demo_data:
            return False, "Blocked: underlying data is synthetic/demo (live feed unavailable) — no trade placed"
    ...
    def generate_high_accuracy_signals(self, symbol, data):
        ...
        try:
            ...
            is_demo_data = bool(getattr(data, "attrs", {}).get("is_demo", False))
            for signal in signals:
                strategy = signal.get("strategy")
                if strategy in self.strategy_performance:
                    self.strategy_performance[strategy]["signals"] += 1
                # SAFETY: tag so execution code can refuse to trade on demo data
                signal["is_demo"] = is_demo_data
    ...
    def generate_strategy_signals(self, symbol, data):
        ...
        try:
            ...
            adx_val = float(data["ADX"].iloc[-1]) if "ADX" in data.columns else 20
            htf_trend = int(data["HTF_Trend"].iloc[-1]) if "HTF_Trend" in data.columns else 0
            ...
            for s in signals:
                ...
                # SAFETY: tag every signal so downstream execution code can
                # refuse to trade on synthetic/demo data (see is_demo checks
                # in auto_execute_signals / execute_trade).
                s["is_demo"] = is_demo_data
    ...
    def auto_execute_signals(self, signals):
        ...
        for signal in signals[:10]:  # Limit to first 10 signals
            ...
            if signal.get("is_demo"):
                st.warning(f"⛔ Skipping {signal['symbol']} — signal was generated from synthetic/demo data (live feed unavailable), refusing to auto-trade it")
                continue
                
            # Risk-based position sizing (falls back to Kelly only if no stop_loss)
            try:
            ...
            if optimal_qty > 0:
                success, msg = self.execute_trade(
                    ...
                    auto_trade=True,
                    strategy=signal.get("strategy"),
                    is_demo_data=signal.get("is_demo", False)
                )
                if success:
                    executed.append(msg)
...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            with tabs[0]:
                st.error(f"Error rendering 'Dashboard' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[1]:
                    ...
                    if signals:
                        ...
                        for idx, s in enumerate(signals[:5]):  # Show only first 5 for better UI
                            ...
                            with col_b:
                                if kelly_sizing:
                                    try:
                                        ...
                                        qty = trader.data_manager.calculate_optimal_position_size(
                                            ...
                                            trader.cash, s["entry"], atr, stop_loss=s.get("stop_loss")
                                        )
                                    except:
                                ...
                                st.write(f"Qty: {qty}")
                                if s.get("is_demo"):
                                    st.caption("⛔ Synthetic/demo data — execution blocked")
                            with col_c:
                                if st.button(f"Execute", key=f"exec_{s['symbol']}_{s['strategy']}_{idx}_{int(time.time())}"):
                                    success, msg = trader.execute_trade(
                                        ...
                                        strategy=s.get("strategy"), is_demo_data=s.get("is_demo", False)
                                    )
                                    if success:
                                        st.success(msg)
                                        st.rerun()
            ...
            with tabs[1]:
                st.error(f"Error rendering 'Signals' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[2]:
            ...
            with tabs[2]:
                st.error(f"Error rendering 'Paper Trading' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[3]:
            ...
            with tabs[3]:
                st.error(f"Error rendering 'Trade History' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[4]:
            ...
            with tabs[4]:
                st.error(f"Error rendering 'RSI Extreme' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[5]:
            ...
            with tabs[5]:
                st.error(f"Error rendering 'Backtest' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[6]:
            ...
            with tabs[6]:
                st.error(f"Error rendering 'Strategies' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[7]:
                            ...
                            with exec_cols[idx % 3]:
                                ...
                                if st.button(
                                    ...
                                    qty = trader.data_manager.calculate_optimal_position_size(
                                        ...
                                        trader.cash, signal["entry"], atr_val, stop_loss=signal.get("stop_loss")
                                    )
                                    success, msg = trader.execute_trade(
        ...
        # Tab 9: Kite Live Charts (NEW TAB)
            ...
            with tabs[7]:
                st.error(f"Error rendering 'High Accuracy Scanner' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[8]:
                st.subheader("📈 Kite Connect Live Charts")
            ...
            with tabs[8]:
                st.error(f"Error rendering 'Kite Live Charts' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[9]:
            ...
            with tabs[9]:
                st.error(f"Error rendering 'Footprint & Profile' tab: {_tab_e}")
                st.code(traceback.format_exc())
        try:
            with tabs[10]:
            ...
            with tabs[10]:
                st.error(f"Error rendering 'Stock Analyze' tab: {_tab_e}")
                st.code(traceback.format_exc())
    except Exception as e:
